...
func (s *Server) StreamEvents(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case event := <-eventsChan:
			tp := topicForEvent(event)
			if !requestedTopics[tp] {
				continue
			}
			lr, err := lazyReaderForEvent(tp, event)
			if err != nil {
			...
			if tp == PayloadAttributesTopic || (tp == HeadTopic && requestedTopics[PayloadAttributesTopic]) {
				ar, err := s.currentPayloadAttributes(ctx)
				if err != nil {
					log.WithError(err).Error("StreamEvents API endpoint failed to get current payload attributes.")
					continue
				}
				if err := es.safeWrite(ar); err != nil {
					log.WithField("event_type", fmt.Sprintf("%v", event.Data)).Warn("Unable to safely write event to stream, shutting down.")
					return
				}
			}
...
func lazyReaderForEvent(eventName string, event *feed.Event) (lazyReader, error) {
	...
	case *ethpbv2.LightClientFinalityUpdateWithVersion:
		return func() io.Reader {
			return jsonMarshalReader(eventName, structs.LightClientFinalityUpdateEventFromV2(v))
		}, nil
	case *ethpbv2.LightClientOptimisticUpdateWithVersion:
		return func() io.Reader {
			return jsonMarshalReader(eventName, structs.LightClientOptimisticUpdateWithVersionFromV2(v))
		}, nil
	case *ethpb.EventChainReorg:
		return func() io.Reader {
